...
function CloseRoomButtonComponent(data) {
    return  `<p class="lead px-3">
                <a data-room-code="${data["roomCode"]}" data-target="alert-close-room-modal" data-bs-dismiss="modal" class="btn modal-handler btn-lg btn-secondary">Close Room</a>
            </p>`
 }
...
function LeaveRoomButtonComponent(userId) {
    return `
        <p class="lead px-3">
            <button data-player-code="${userId}" data-target="alert-leave-room-modal" data-bs-dismiss="modal" class="btn modal-handler btn-lg btn-secondary">Leave Room</button>
        </p>
        `
}
...
async function TournamentActionsComponent(data, roomCode) {
    ...
    else if (data["round"] == 1 && data["roomStatus"] != 3)
    {
        ...
        btnSection.classList.add("d-flex", "justify-content-end", "w-100")
        btnSection.innerHTML = `
            <button data-player-code="${data}" data-target="alert-leave-room-modal" data-bs-dismiss="modal"  class="btn modal-handler btn-lg btn-secondary">Leave Room</button>
        `
        elementActions.appendChild(btnSection)
    }
