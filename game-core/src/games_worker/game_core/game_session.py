...
class GameSession:
	...
	async def add_player_channels(self):
		...
		self.tasks_movement_player.append(asyncio.create_task(self.update_ball_position()))
			...
			if game.isSinglePlayer is True:
				for player in players:
					if player.is_connected == True:
						...
						break
			if are_players_connected or time >= 60:
				break
	...
	async def game_loop(self):
		...
		while True:
			if await self.update_ball_position() == self.ball_collision:
				self.ball_collision = False
				break
